...
exports.insertUser = async(fullname, email, hash) => {
    const result = await db.connection.execute('insert into accounts (PASSWORD,FULLNAME, EMAIL, IMAGE, ROLE, ACTIVE, VERIFIED) VALUES (?,?,?,?,?,?,?)',[hash, fullname, email,'default-user.png', "student", 1, '0']);
    return result[0];
}